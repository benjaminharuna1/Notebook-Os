...
<div class="fixed bottom-4 right-4 z-50 flex flex-col items-end">
  ...
  {/if}
  <button
    onclick={() => (active.length > 0 ? (open = !open) : null)}
    aria-label="Show running actions"
    aria-expanded={open}
    class="relative flex items-center gap-2 rounded-full border border-slate-200 bg-white py-2 pl-3.5 pr-2.5 text-sm font-medium text-slate-700 shadow-lg transition-colors {active.length === 0 ? 'cursor-default opacity-60' : 'hover:bg-slate-50'}"
  >
    {#if isBusy}
